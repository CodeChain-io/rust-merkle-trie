...
impl<'a> TrieMut for TrieDBMut<'a> {
    fn insert(&mut self, key: &[u8], value: &[u8]) -> crate::Result<Option<DBValue>> {
        ...
        let mut old_val = None;
        let cur_hash = *self.root;
        *self.root = self.insert_aux(NibbleSlice::new(path.as_ref()), value, Some(cur_hash), &mut old_val)?;
    ...
    fn remove(&mut self, key: &[u8]) -> crate::Result<Option<DBValue>> {
        ...
        *self.root = match self.remove_aux(&NibbleSlice::new(path.as_ref()), Some(cur_hash), &mut old_val)? {
            Some(hash) => hash,
            None => BLAKE_NULL_RLP,
        };
